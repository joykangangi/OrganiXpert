...
def preprocess_before_inference(data):
    ...
    # data['potassium'] = scaler.fit_transform(data[['potassium']])
    # data['phosphorus'] = scaler.fit_transform(data[['phosphorus']])
    # MinMaxScaler() returned only zero values hence we decided to manually normalize the values
    # normalize values manually without using MinMaxScaler
    data['nitrogen'] = (data['nitrogen'] - 0) / (100 - 0)
    data['potassium'] = (data['potassium'] - 0) / (100 - 0)
    data['phosphorus'] = (data['phosphorus'] - 0) / (100 - 0)
    return data
